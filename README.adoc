...
to create it. Afterwards an archive file with the contents can be found within the `target` subdirectory.
The uncompressed P2 repository can be found at `features/net.enilink.komma.updatesite/target/repository`.
...
__File > Import > Maven > Existing Mavne Projects__.
