...
public interface IContentHandler {
	/**
	 * A registry of content handlers based on a priority order; lower values
	 * indicate have higher priority.
	 */
	interface Registry extends SortedMap<Integer, List<IContentHandler>> {
		/**
		...
		/**
		 * Adds an additional content handler with the given priority to the
		 * map. If there is already a list for the given priority in the map,
		 * the handler is added to that list. Otherwise, a new list containing
		 * the handler is created and put into the map.
		 * 
		 * @param priority
		 ...
		 */
		void put(int priority, IContentHandler contentHandler);

		/**
		 * Returns a read only list of all the content handlers in the map in
		 * priority order.
		 * 
		 * @return a read only list of all the content handlers in the map in
		 *         priority order.
		 */
	...
	enum ByteOrderMark {
		...
		/**
		 * Returns the bytes associated with this byte order mark. This value
		 * will be identical to the corresponding constant for
		 ...
		 * @return the byte order mark at the start of the input stream, or
		 *         <code>null</code> if there isn't one.
		 * @throws IOException
		 *             if there is a problem reading from the input stream.
		 */
		public static ByteOrderMark read(InputStream inputStream)
				throws IOException {
	...
	Map<String, Object> INVALID_CONTENT_DESCRIPTION = Collections
			.unmodifiableMap(ContentHandler
					.createContentDescription(Validity.INVALID));

	/**
	 * Returns a map of properties that describe the content of the given URI's
	 * corresponding input stream. The {@link #VALIDITY_PROPERTY validity
	 * property} will always be present to indicate the status. The
	 * {@link #CONTENT_TYPE_PROPERTY content type property} too will always be
	 * present, except when the validity property is {@link Validity#INVALID}.
	 * The option {@link #OPTION_REQUESTED_PROPERTIES} can be used to specify
	 * the set of additional properties that should appear in the result. If
	 * this option is not present, all properties this handler can compute will
	 * be returned. The context map is used to cache results that can be shared
	 * between content handler implementations. For example, once a content
	 ...
	 * @param inputStream
	 *            the input stream associated with the given URI.
	 * @param options
	 *            a map of options to direct what kind of description is needed.
	 * @param context
	 *            a map of contextual information that content handlers use to
	 *            store partially computed results.
	 * @return a map of properties that describe the content of the given URI's
	 *         corresponding input stream.
	 * @throws IOException
	 *             if there is a problem reading the stream.
	 */
	Map<String, ?> contentDescription(URI uri, InputStream inputStream,
