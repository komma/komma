...
public class SparqlUpdateExecutor {
	private final Logger logger = LoggerFactory
			.getLogger(SparqlUpdateExecutor.class);
	...
	static final Collection<IStatementPattern> ANY_STATEMENT = Collections
			.<IStatementPattern> singleton(new net.enilink.komma.core.StatementPattern(
					null, null, null));
	...
	static class UpdateContext {
		...
		UpdateContext(UpdateExpr updateExpr, String baseURI,
				IReference[] readContexts, IReference[] modifyContexts,
	...
	public SparqlUpdateExecutor(IDataManager dm,
			SesameValueConverter valueConverter, ValueFactory vf) {
		this.dm = dm;
		this.valueConverter = valueConverter;
		this.vf = vf;
	}

	public void executeUpdate(UpdateExpr updateExpr, String baseURI,
			IReference[] readContexts, IReference[] modifyContexts,
			IBindings<IValue> bindings, boolean includeInferred)
			throws KommaException {
		UpdateContext uc = new UpdateContext(updateExpr, baseURI, readContexts,
	...
	protected void executeLoad(Load load, UpdateContext uc)
		...
		Value graph = load.getGraph() != null ? load.getGraph().getValue()
				: null;
	...
	protected void executeCreate(Create create, UpdateContext uc)
		...
		if (graphValue instanceof Resource) {
			...
			IDataManagerQuery<?> contextQuery = dm.createQuery(
					"ask { graph ?g { ?s ?p ?o } }", null, false);
			try (IExtendedIterator<?> result = contextQuery.evaluate()) {
				if (Boolean.TRUE.equals(result.next())) {
					throw new KommaException("Named graph " + namedGraph
							+ " already exists. ");
				}
			}
		}
	...
	protected void executeCopy(Copy copy, UpdateContext uc)
		...
		try (IExtendedIterator<IStatement> stmts = dm.match(null, null, null,
				uc.includeInferred, valueConverter.fromSesame(source))) {
				...
				if (Scope.NAMED_CONTEXTS.equals(scope)) {
					IDataManagerQuery<?> contextQuery = dm
							.createQuery(
									...
									null, false);
					for (IReference ctx : contextQuery.evaluate()
							.mapWith(new IMap<Object, IReference>() {
								@Override
								public IReference map(Object value) {
									return valueConverter
											.fromSesame((Resource) ((IBindings<?>) value)
													.get("g"));
								}
							}).toList()) {
	...
	protected IExtendedIterator<IBindings<IValue>> evaluateConstant(
			TupleExpr tupleExpr, UpdateContext uc) {
		if (!(tupleExpr instanceof QueryRoot)) {
			// allow the optimizers to modify the actual root node
		...
		EvaluationStrategyImpl strategy = new EvaluationStrategyImpl(
				new TripleSource() {
					...
					public CloseableIteration<? extends Statement, QueryEvaluationException> getStatements(
							Resource s, URI arg1, Value p, Resource... ctxs)
							throws QueryEvaluationException {
						return new EmptyIteration<>();
					}
				});
		Dataset ds = valueConverter.createDataset(uc.readContexts,
				uc.modifyContexts);
		BindingSet bs = valueConverter.toSesame(uc.bindings);
		new BindingAssigner().optimize(tupleExpr, ds, bs);
		try {
			return new SesameResult<BindingSet, IBindings<IValue>>(
					strategy.evaluate(tupleExpr, bs)) {
				@Override
				protected IBindings<IValue> convert(BindingSet element)
						throws Exception {
					LinkedHashBindings<IValue> result = new LinkedHashBindings<IValue>(
							element.size());
					for (String name : element.getBindingNames()) {
						IValue value = valueConverter.fromSesame(element
								.getValue(name));
						if (value != null) {
	...
	protected void executeInsertData(InsertData insertDataExpr, UpdateContext uc)
		...
		try (IExtendedIterator<IBindings<IValue>> results = evaluateConstant(
				insertExpr, uc)) {
			dm.add(results
					.mapWith(new IMap<IBindings<IValue>, net.enilink.komma.core.Statement>() {
						@Override
						public net.enilink.komma.core.Statement map(
								IBindings<IValue> bs) {
							IReference subject = (IReference) bs.get("subject");
							IReference predicate = (IReference) bs
									.get("predicate");
							IValue object = bs.get("object");
							IReference context = (IReference) bs.get("context");
	...
	protected void executeDeleteData(DeleteData deleteDataExpr, UpdateContext uc)
		...
		try (IExtendedIterator<IBindings<IValue>> results = evaluateConstant(
				deleteExpr, uc)) {
			dm.remove(
					results.mapWith(new IMap<IBindings<IValue>, net.enilink.komma.core.Statement>() {
						@Override
						public net.enilink.komma.core.Statement map(
								IBindings<IValue> bs) {
							IReference subject = (IReference) bs.get("subject");
	...
	protected IExtendedIterator<?> evaluateSparql(TupleExpr tupleExpr,
			UpdateContext uc) {
		try {
			SparqlTupleExprRenderer renderer = new SparqlTupleExprRenderer();
			String sparql = renderer.render(tupleExpr);
			return dm.createQuery(sparql, uc.baseURI, uc.includeInferred,
					uc.readContexts).evaluate();
		} catch (KommaException e) {
			throw e;
		} catch (Exception e) {
			throw new KommaException(e);
	...
	protected void executeModify(Modify modify, UpdateContext uc)
		...
		try (IExtendedIterator<IBindings<IValue>> sourceBindings = evaluateWhereClause(
				whereClause, uc)) {
	...
	private IExtendedIterator<IBindings<IValue>> evaluateWhereClause(
			final TupleExpr whereClause, final UpdateContext uc)
			throws KommaException {
		return evaluateSparql(whereClause, uc).mapWith(
				new IMap<Object, IBindings<IValue>>() {
					@Override
					public IBindings<IValue> map(Object value) {
						...
						if (whereClause instanceof SingletonSet
								&& sourceBindings.getKeys().isEmpty()
								&& uc.bindings != null) {
							...
							if (uniqueBindings.size() > 0) {
								...
								for (String bindingName : sourceBindings
										.getKeys()) {
									mergedSet.put(bindingName,
											sourceBindings.get(bindingName));
								}
								for (String bindingName : uniqueBindings) {
									mergedSet.put(bindingName,
											uc.bindings.get(bindingName));
								}
								return mergedSet;
							}
	...
	private void deleteBoundTriples(IBindings<IValue> whereBinding,
			TupleExpr deleteClause, UpdateContext uc) throws KommaException {
		if (deleteClause != null) {
			...
			for (StatementPattern deletePattern : deletePatterns) {
				IReference subject = (IReference) getValueForVar(
						deletePattern.getSubjectVar(), whereBinding);
				IReference predicate = (IReference) getValueForVar(
						deletePattern.getPredicateVar(), whereBinding);
				IValue object = getValueForVar(deletePattern.getObjectVar(),
						whereBinding);

				IReference context = null;
				if (deletePattern.getContextVar() != null) {
					context = (IReference) getValueForVar(
							deletePattern.getContextVar(), whereBinding);
				}
				...
				net.enilink.komma.core.StatementPattern stmt = new net.enilink.komma.core.StatementPattern(
	...
	private void insertBoundTriples(IBindings<IValue> whereBinding,
			TupleExpr insertClause, UpdateContext uc) throws KommaException {
		if (insertClause != null) {
			...
			for (StatementPattern insertPattern : insertPatterns) {
				net.enilink.komma.core.Statement toBeInserted = createStatementFromPattern(
	...
	private net.enilink.komma.core.Statement createStatementFromPattern(
			StatementPattern pattern, IBindings<IValue> sourceBindings,
			Map<String, IReference> bnodeMapping) throws KommaException {
		...
		if (pattern.getSubjectVar().hasValue()) {
			subject = valueConverter.fromSesame((Resource) pattern
					.getSubjectVar().getValue());
		} else {
			subject = (IReference) sourceBindings.get(pattern.getSubjectVar()
					.getName());
			if (subject == null && pattern.getSubjectVar().isAnonymous()) {
				...
				if (subject == null) {
					...
					bnodeMapping
							.put(pattern.getSubjectVar().getName(), subject);
				}
			}
		}

		if (pattern.getPredicateVar().hasValue()) {
			predicate = valueConverter.fromSesame((Resource) pattern
					.getPredicateVar().getValue());
		} else {
			predicate = (IReference) sourceBindings.get(pattern
					.getPredicateVar().getName());
		}

		if (pattern.getObjectVar().hasValue()) {
			object = valueConverter.fromSesame(pattern.getObjectVar()
					.getValue());
		} else {
			...
			if (object == null && pattern.getObjectVar().isAnonymous()) {
				...
				if (object == null) {
					object = dm.blankNode();
					bnodeMapping.put(pattern.getObjectVar().getName(),
							(IReference) object);
				}
			}
		}
		...
		if (pattern.getContextVar() != null) {
			if (pattern.getContextVar().hasValue()) {
				context = valueConverter.fromSesame((Resource) pattern
						.getContextVar().getValue());
			} else {
				context = (IReference) sourceBindings.get(pattern
		...
		if (subject != null && predicate != null && object != null) {
			if (context != null) {
				st = new net.enilink.komma.core.Statement(subject, predicate,
						object, context);
			} else {
				st = new net.enilink.komma.core.Statement(subject, predicate,
						object);
	...
	private IValue getValueForVar(Var var, IBindings<IValue> bindings)
			throws KommaException {
		if (var.hasValue()) {
