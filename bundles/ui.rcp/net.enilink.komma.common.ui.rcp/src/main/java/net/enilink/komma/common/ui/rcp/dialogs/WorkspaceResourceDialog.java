 ...
 * @since 2.2.0
 */
public class WorkspaceResourceDialog extends ElementTreeSelectionDialog
	...
	public static IContainer[] openFolderSelection(Shell parent, String title,
		...
		dialog.setTitle(title != null ? title : CommonUIPlugin.INSTANCE
				.getString("_UI_FolderSelection_title"));
		...
		dialog.loadContents();
		return dialog.open() == Window.OK ? dialog.getSelectedContainers()
				: new IContainer[0];
	}
	...
	public WorkspaceResourceDialog(Shell parent, ILabelProvider labelProvider,
			...
			public boolean select(Viewer viewer, Object parentElement,
				...
				if (element instanceof IResource) {
					...
					return workspaceResource.isAccessible()
							&& (WorkspaceResourceDialog.this.showFiles || workspaceResource
	...
	protected void createNewFolderControl(Composite parent) {
		...
		newFolderButton.setText(CommonUIPlugin.INSTANCE
				.getString("_UI_NewFolder_label"));
	...
	protected void createFileControl(Composite parent) {
		...
		fileLabel.setText(CommonUIPlugin.INSTANCE
				.getString("_UI_FileName_label"));
	...
	public IStatus validate(Object[] selectedElements) {
		...
		for (int i = 0; i < selectedElements.length; i++) {
			if (selectedElements[i] instanceof IContainer) {
				enableOK = !showFiles
						|| (isShowFileControl() && fileText.getText().trim()
								.length() > 0);
		...
		return enableOK ? new Status(IStatus.OK, "org.eclipse.emf.common.ui",
				0, "", null) : new Status(IStatus.ERROR,
	...
	public String getFileText() {
		return fileText != null && !fileText.isDisposed() ? fileText.getText()
				: fileTextContent;
	}
